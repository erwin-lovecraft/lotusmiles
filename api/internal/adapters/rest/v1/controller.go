...
func (s Controller) GetMyAccrualRequests(c lit.Context) error {
	var req AccrualRequestFilter
	if err := c.Bind(&req); err != nil {
		return err
	}

	data, total, err := s.mileage.GetMyAccrualRequests(c, req.toApplicationDTO())
	if err != nil {
...
func (s Controller) GetMyMileageLedgers(c lit.Context) error {
	var req MileageLedgerFilter
	if err := c.Bind(&req); err != nil {
		return err
	}

	data, total, err := s.mileage.GetMyMileageLedgers(c, req.toApplicationDTO())
	if err != nil {
		return convertErr(err)
	}
